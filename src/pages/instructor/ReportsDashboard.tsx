...
// pages/instructor/reports-dashboard.tsx
...
import { ParticipantReport, EnrollmentStatus } from '@/types';
import { Button } from '@/components/ui/button';

const mockReports: ParticipantReport[] = [];
...
export function ReportsDashboard() {
  ...
  const stats = {
    ...
    yet: mockReports.filter((r) => r.status === 'Yet to Start').length,
    progress: mockReports.filter((r) => r.status === 'In Progress').length,
    done: mockReports.filter((r) => r.status === 'Completed').length,
  };
  ...
  return (
    <div className="p-6">
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <Button onClick={() => setStatusFilter('Yet to Start')}>
          <Clock /> {stats.yet}
        </Button>
        ...
        <Button onClick={() => setStatusFilter('Completed')}>
          <CheckCircle /> {stats.done}
        </Button>
      </div>

      {filtered.map((r) => (
        <div key={r.id} className="border p-3 rounded mb-2">
          <strong>{r.learnerName}</strong> – {r.courseName} – {r.status}
        </div>
      ))}
    </div>
  );
